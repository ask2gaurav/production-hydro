...
#!/bin/bash
# ==============================================================================# Docker MongoDB to Google Drive Weekly Backup Script# ==============================================================================# Description: Dumps a Dockerized MongoDB with authentication, compresses the#              output, uploads it to Google Drive via rclone, and performs#              retention cleanup both locally and on cloud storage.# ==============================================================================
# ------------------------------------------------------------------------------# 1. Configuration Variables# ------------------------------------------------------------------------------
CONTAINER_NAME="mongodb"                 # Name of your MongoDB Docker container
MONGO_USER="mongouser"                   # MongoDB Administrative Username
MONGO_PASS="mongopass"                   # MongoDB Administrative Password
AUTH_DB="admin"                          # Authentication database (usually 'admin')

LOCAL_BACKUP_DIR="/opt/mongo_backups"    # Host path where temporary backups are stored

GDRIVE_REMOTE="gdrive"                   # Name of the rclone remote configured
GDRIVE_FOLDER="MongoBackups"             # Target directory name in Google Drive
# Custom API Credentials (Required as shared client_id is retiring in 2026)
GDRIVE_CLIENT_ID="YOUR_CUSTOM_CLIENT_ID_HERE"
GDRIVE_CLIENT_SECRET="YOUR_CUSTOM_CLIENT_SECRET_HERE"

RETENTION_DAYS=30                        # Age threshold to clean up old local copies
# ------------------------------------------------------------------------------# 2. Setup Internal States & Logging# ------------------------------------------------------------------------------
TIMESTAMP=$(date +"%Y%m%d_%H%M%S")
BACKUP_NAME="mongodb_backup_${TIMESTAMP}.archive.gz"
...
# Executing mongodump inside the container and piping the compressed stream directly# to a local compressed archive file on the host architecture.if docker exec -i "${CONTAINER_NAME}" mongodump \
    --username "${MONGO_USER}" \
    --password "${MONGO_PASS}" \
...
# Explicitly passing the custom client credentials dynamically to ensure # authentication stability over the retired shared infrastructure.if rclone copy "${LOCAL_FILE_PATH}" "${GDRIVE_REMOTE}:${GDRIVE_FOLDER}" \
...
find "${LOCAL_BACKUP_DIR}" -type f -name "mongodb_backup_*.archive.gz" -mtime +"${RETENTION_DAYS}" -exec rm -v {} \;
...
FILE_LIST=$(rclone lsf --format "t|p" "${GDRIVE_REMOTE}:${GDRIVE_FOLDER}" \
    --drive-client-id "${GDRIVE_CLIENT_ID}" \
    --drive-client-secret "${GDRIVE_CLIENT_SECRET}" 2>/dev/null | grep "mongodb_backup_" | sort)
# Count how many total backup files currently exist in the cloud directory

FILE_COUNT=$(echo "$FILE_LIST" | grep -c "mongodb_backup_")

echo "Current cloud backup count: ${FILE_COUNT} (Target Threshold: ${RETENTION_DAYS})"
# Safety Check: Only proceed with removing files if the total file count strictly exceeds # the retention threshold, ensuring you always keep a rolling baseline of backups.if [ "$FILE_COUNT" -gt "$RETENTION_DAYS" ]; then
    ...
    fielse
    echo "Cloud backup count (${FILE_COUNT}) is within safety threshold (<= ${RETENTION_DAYS}). No cloud files will be removed."fi
