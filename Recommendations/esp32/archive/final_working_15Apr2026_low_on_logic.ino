...
void setup() {
	...
	server.on("/", HTTP_GET, [](AsyncWebServerRequest* request) {
		String outputMessage = "{\"temp\": " + String(readSensorTemperature) + ", \"water_hl\": " + String(readLH) + ", \"water_ll\": " + String(readLL) + ", \"blower\": " + String(1-readBlower) + ", \"flush_valve\": " + String(1-readFlush) + ", \"water_in_valve\": " + String(1-readWaterInSq) + ", \"pump\": " + String(1-readWaterPumpOut) + ", \"flush_button_hardware\": " + String(1-flushButtonHardwareHit) + ",\"heater\": " + String(1-readHeater) + "}";
...
void END_SESSION(){
	for (int i = 0; i < outputPinsLen; i++) {
		digitalWrite(outputPins[i], MY_ON); // set all pins as off
	}
}
void loop() {
