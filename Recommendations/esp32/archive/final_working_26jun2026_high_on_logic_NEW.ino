...
void handle_hardware_button() {
	...
	// Button just pressed
	if (currentState == MY_ON && hwButtonPrevState == MY_OFF) {
...
void END_SESSION(){
	for (int i = 0; i < outputPinsLen; i++) {
		if(outputPins[i] != HEATER){ // keep heater off at end of session, other pins off at end of session
			digitalWrite(outputPins[i], MY_ON); // set all pins as off by default except heater
		}else{
			digitalWrite(outputPins[i], MY_OFF); // keep heater off at end of session
		}
	}
}
