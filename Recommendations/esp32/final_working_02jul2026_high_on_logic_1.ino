...
unsigned long hwButtonPressEnd = 0;
...
void handle_hardware_button() {
	...
	if (currentState == MY_ON && hwButtonPrevState == MY_OFF) {
		hwButtonPressEnd = millis();
		hwButtonLongPressFired = true;
		//delay (500);
		}

	long hwButtonDuration = hwButtonPressEnd - hwButtonPressStart;


	//if (currentState == MY_ON && hwButtonLongPressFired) {

			if (hwButtonDuration << LONG_PRESS_DURATION){
